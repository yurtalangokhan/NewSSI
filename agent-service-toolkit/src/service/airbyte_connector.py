...
def _infer_category(definition: Dict[str, Any]) -> str:
    ...
    if "database" in repo or any(
        db in repo
        for db in ["postgres", "mysql", "mssql", "mongo", "oracle", "mariadb"]
    ):
        return "database"
...
async def search_connectors(query: str) -> List[ConnectorInfo]:
    ...
    return [
        c
        for c in connectors
        if q in c.name.lower() or q in c.display_name.lower()
    ]
...
async def get_connector_spec(connector_name: str) -> ConnectorSpec:
    ...
    return ConnectorSpec(
        ...
        documentation_url=spec_data.get("documentationUrl")
        or connector.documentation_url,
    )
...
async def get_available_streams(
    ...
    try:
        ...
        return [
            s.get("stream", {}).get("name", "")
            for s in streams
            if s.get("stream", {}).get("name")
        ]
    finally:
        try:
            await client.delete_source(source_id)
        ...
        if job_status != "succeeded":
            ...
            try:
                ...
                if attempts:
                    ...
                    if failures:
                        messages = [f.get("failureOrigin", "") + ": " + f.get("externalMessage", f.get("internalMessage", "")) for f in failures]
                    ...
                    if not failure_detail:
                        ...
                        error_lines = [l for l in logs if isinstance(l, str) and ("error" in l.lower() or "exception" in l.lower())]
                        if error_lines:
