...
async def invoke(
    ...
    The user_id can also be automatically extracted from the x-supabase-access-token header.
    """
    supabase_token = request.headers.get("x-supabase-access-token")
    supabase_user_id = extract_user_id_from_supabase_token(supabase_token) if supabase_token else None
    ...
    kwargs, run_id = await _handle_input(user_input, agent, supabase_user_id)
...
async def stream(
    ...
    return StreamingResponse(
        message_generator(user_input, agent_id, supabase_user_id),
