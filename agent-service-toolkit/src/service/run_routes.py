...
from service.active_runs import register_run, unregister_run, cancel_run, cancel_all_for_thread, is_cancelled, get_run_context
from service.auth import extract_user_id_from_supabase_token, verify_bearer
...
async def _force_close_llm_connection(config: dict | None = None) -> None:
    ...
    try:
        ...
        if old_httpx is None or not isinstance(old_httpx, _httpx.AsyncClient):
            print(f"[_force_close_llm_connection] No httpx client on {type(ollama_aclient).__name__}")
            return
...
async def _persist_partial_messages(
    ...
    print(f"[_persist_partial_messages] all_messages types: {[m.get('type') for m in all_messages]}")
    ...
    try:
        print(f"[_persist_partial_messages] called: agent={type(agent)}, config={config is not None}, all_messages={len(all_messages)}, checkpointer={checkpointer is not None}")
        logger.debug(f"[_persist_partial_messages] called: agent={type(agent)}, config={config is not None}, all_messages={len(all_messages)}, checkpointer={checkpointer is not None}")
        # Only persist AI / tool messages (the human message is already in
        # the checkpoint from the invoke).
        partial_lc_messages = [
            _dict_to_message(m)
            for m in all_messages
            if m.get("type") in ("ai", "tool")
        ]
        ...
        print(f"[_persist_partial_messages] graph: {type(graph)}, has aupdate_state: {hasattr(graph, 'aupdate_state')}")
        logger.debug(f"[_persist_partial_messages] graph: {type(graph)}, has aupdate_state: {hasattr(graph, 'aupdate_state')}")
        ...
        if graph and hasattr(graph, "aupdate_state"):
            ...
            print(f"[_persist_partial_messages] calling aupdate_state with {len(partial_lc_messages)} messages")
            logger.debug(f"[_persist_partial_messages] calling aupdate_state with {len(partial_lc_messages)} messages")
            await graph.aupdate_state(
                update_config,
                {"messages": partial_lc_messages},
                as_node=node_name,
            )
            print(f"[_persist_partial_messages] Persisted {len(partial_lc_messages)} partial messages to checkpoint for thread {config.get('configurable', {}).get('thread_id')}")
            logger.info(
            ...
            print("[_persist_partial_messages] Cannot persist: graph does not support aupdate_state")
            logger.warning(
                "Cannot persist partial messages: graph does not support aupdate_state"
            )
    except Exception as exc:
...
def _schedule_persist(
    ...
    async def _do_persist():
        print(f"[_schedule_persist/_do_persist] Persist task started for thread_id={thread_id}, run_id={run_id}, {len(all_messages)} messages")
        logger.debug(f"[_schedule_persist/_do_persist] Persist task started for thread_id={thread_id}, run_id={run_id}, {len(all_messages)} messages")
        try:
            await _persist_partial_messages(agent, config, all_messages, checkpointer)
            print(f"[_schedule_persist/_do_persist] Partial messages persisted successfully for thread_id={thread_id}, run_id={run_id}")
            logger.debug(f"[_schedule_persist/_do_persist] Partial messages persisted successfully for thread_id={thread_id}, run_id={run_id}")
        except Exception as exc:
            print(f"Scheduled persist failed for thread_id={thread_id}, run_id={run_id}: {exc}")
            logger.error(f"Scheduled persist failed for thread_id={thread_id}, run_id={run_id}: {exc}")
        finally:
            unregister_run(thread_id, run_id)

    try:
        print(f"[_schedule_persist] Scheduling persist for thread_id={thread_id}, run_id={run_id}, {len(all_messages)} messages")
        logger.debug(f"[_schedule_persist] Scheduling persist for thread_id={thread_id}, run_id={run_id}, {len(all_messages)} messages")
        ...
        print(f"No running event loop for scheduled persist (thread_id={thread_id}, run_id={run_id})")
        logger.warning(f"No running event loop for scheduled persist (thread_id={thread_id}, run_id={run_id})")
        unregister_run(thread_id, run_id)
...
async def stream_run(
    ...
    User ID is automatically extracted from x-supabase-access-token header if present.
    """
    ...
    supabase_token = request_obj.headers.get("x-supabase-access-token")
    print(f"[DEBUG] supabase_token: {supabase_token[:50] if supabase_token else 'None'}...")

    supabase_user_id = (
        extract_user_id_from_supabase_token(supabase_token) if supabase_token else None
    )
    print(f"[DEBUG] extracted supabase_user_id: {supabase_user_id}")

    if supabase_user_id:
        logger.info(f"stream_run: Using user_id from Supabase token: {supabase_user_id}")
    else:
        logger.info("stream_run: No Supabase token found, user_id will not be set")
    ...
    try:
        ...
        if existing_thread:
            ...
            logger.info(
                f"Created new thread {thread_id} with assistant_id {request.assistant_id}"
            )
    except Exception as e:
    ...
    if supabase_user_id:
        ...
        logger.info(f"stream_run: Set user_id in configurable: {supabase_user_id}")
    ...
    async def event_generator():  # noqa: C901  — complex but self-contained
        ...
        try:
                ...
                while True:
                    try:
                        ...
                        if _check_cancelled() or await request_obj.is_disconnected():
                            if not cancel_event.is_set():
                                cancel_event.set()
                            print(f"[event_generator] Cancel/disconnect detected for run {run_id} (astream)")
                            if _agent_task and not _agent_task.done():
                                _agent_task.cancel()
                                print(f"[event_generator] Cancelled agent task for run {run_id}")
                            break
                    ...
                    if (
                        ...
                        and chunk.response_metadata.get("done")
                    ):
            ...
            print(f"[DEBUG] event_generator: Sending values event with {len(all_messages)} messages")
            ...
            print(f"[event_generator finally] Entered for thread_id={thread_id}, run_id={run_id}, cancel_event={cancel_event.is_set()}, messages={len(all_messages)}")
            logger.debug(f"[event_generator finally] Entered for thread_id={thread_id}, run_id={run_id}, cancel_event={cancel_event.is_set()}, messages={len(all_messages)}")
            ...
            if current_message_id and current_message_content:
                ...
                if not any(m.get("id") == current_message_id for m in all_messages):
                    ...
                    print(f"[event_generator finally] Flushed partial AI message id={current_message_id}, len={len(current_message_content)}")

            print(f"[event_generator finally] all_messages types after flush: {[m.get('type') for m in all_messages]}")
...
async def cancel_run_endpoint(
    ...
    print(f"[cancel_run_endpoint] RunContext: ctx is not None: {ctx is not None}, messages: {len(ctx.all_messages) if ctx else 'n/a'}, checkpointer: {ctx.checkpointer is not None if ctx else 'n/a'}")
    if ctx and ctx.all_messages:
        print(f"[cancel_run_endpoint] ctx.all_messages types: {[m.get('type') for m in ctx.all_messages]}")
    logger.debug(f"[cancel_run_endpoint] RunContext: ctx is not None: {ctx is not None}, messages: {len(ctx.all_messages) if ctx else 'n/a'}, checkpointer: {ctx.checkpointer is not None if ctx else 'n/a'}")
    ...
    try: 
        if ctx and ctx.all_messages and ctx.checkpointer:
            ...
            if ai_messages:
                print(f"Cancel endpoint: persisting {len(ai_messages)} partial messages for run {run_id}")
                logger.info(f"Cancel endpoint: persisting {len(ai_messages)} partial messages for run {run_id}")
                print(f"Cancel endpoint: message types: {[m.get('type') for m in ctx.all_messages]}")
                try:
            ...
            print(f"Cancel endpoint: no partial messages to persist (ctx={ctx is not None}, msgs={len(ctx.all_messages) if ctx else 0}, cp={ctx.checkpointer is not None if ctx else False})")
            logger.debug(f"Cancel endpoint: no partial messages to persist (ctx={ctx is not None}, msgs={len(ctx.all_messages) if ctx else 0}, cp={ctx.checkpointer is not None if ctx else False})")
    except Exception as exc:
...
async def get_thread_history(
    ...
    try:
        ...
        async for checkpoint in saver.alist(
            config, limit=request.limit, before=request.before
        ):
            raw_values = (
            ...
            if checkpoint.parent_config and checkpoint.parent_config.get("configurable"):
                parent_checkpoint = {
                    ...
                    "checkpoint_id": checkpoint.parent_config["configurable"].get(
                        "checkpoint_id"
                    ),
            ...
            state = ThreadState(
                ...
                created_at=(
                    checkpoint.metadata.get("created_at") if checkpoint.metadata else None
                ),
