...
from service.auth import extract_user_id_from_supabase_token
...
async def get_configured_agent(agent_id: str, agent_config: dict) -> AgentGraph:
    ...
    if isinstance(graph_like, LazyLoadingAgent):
        ...
        if hasattr(graph_like, "create_configured_graph") and "sub_agents" in merged_config:
            ...
            if sub_agents_config and isinstance(sub_agents_config, list) and len(sub_agents_config) > 0:
...
async def _handle_input(
    ...
    Args:
        ...
        supabase_user_id: User ID extracted from x-supabase-access-token header (takes priority)
    """
    run_id = uuid4()
    ...
    # Priority: 1) supabase_user_id from token, 2) user_input.user_id, 3) generate new UUID
    user_id = supabase_user_id or user_input.user_id or str(uuid4())
    ...
    if interrupted_tasks:
        ...
        raise HTTPException(status_code=400, detail="One of 'message' or 'messages' must be provided.")
