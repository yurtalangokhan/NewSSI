...
def load_api_versioning_config(
    ...
    *,
    ...
    resolved_path = (
        Path(config_path) if config_path is not None else find_api_versioning_config()
    )
    ...
    service_config = _as_mapping(
        _as_mapping(raw_config.get("services")).get(service_name)
    )
...
def load_api_prefix(
    ...
    *,
    ...
    return load_api_versioning_config(
        service_name,
...
def _validate_config(config: ApiVersioningConfig) -> ApiVersioningConfig:
    if config.default_version not in config.supported_versions:
        msg = "api default_version must be included in supported_versions"
        raise ValueError(msg)
    for version in config.deprecated_versions:
        if version not in config.supported_versions:
            msg = "deprecated_versions must be included in supported_versions"
            raise ValueError(msg)
    return config


API_VERSIONING = load_api_versioning_config("rag-service")
