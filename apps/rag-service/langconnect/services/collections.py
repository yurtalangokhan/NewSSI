...
Business-logic layer for collection and document orchestration.
Collection metadata (name, owner, etc.) lives in Postgres (langchain_pg_collection).
All embedding / chunk data is stored and queried from Milvus.
Low-level database code (repositories, connection helpers) lives in
``langconnect.database``.
"""
