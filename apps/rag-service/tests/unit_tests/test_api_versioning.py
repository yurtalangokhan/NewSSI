...
""",
    ...
    config = load_api_versioning_config("rag-service", config_path=config_path)
...
""",
    ...
    with pytest.raises(ValueError, match="default_version"):
        load_api_versioning_config("rag-service", config_path=config_path)


def test_user_service_api_prefix_can_differ_from_local_service(tmp_path: Path) -> None:
    from langconnect.api_versioning import load_api_prefix
