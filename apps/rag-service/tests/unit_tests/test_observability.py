...
def test_json_log_formatter_emits_service_event_and_extra_fields() -> None:
    from langconnect.observability import JsonLogFormatter

    record = logging.LogRecord(
        name="langconnect.database.postgres.startup",
        level=logging.INFO,
        pathname=__file__,
        lineno=12,
        msg="LangConnect database migrations completed.",
        args=(),
        exc_info=None,
    )
    record.service = "rag-service"
    ...
    payload = json.loads(JsonLogFormatter(service_name="rag-service").format(record))
    ...
    assert payload["message"] == "LangConnect database migrations completed."


def test_text_log_formatter_uses_color_and_operator_fields() -> None:
    from langconnect.observability import TextLogFormatter

    record = logging.LogRecord(
        name="langconnect.server",
        level=logging.ERROR,
        pathname=__file__,
        lineno=12,
        msg="postgres is unavailable during startup migrate.",
        args=(),
        exc_info=None,
    )
...
def test_configure_logging_installs_single_service_formatter() -> None:
    from langconnect.observability import configure_logging, get_logger
    ...
    try:
        configure_logging(
            service_name="rag-service", log_level="INFO", log_format="json"
        )
        configure_logging(
            service_name="rag-service", log_level="INFO", log_format="json"
        )

        logger = get_logger("langconnect.server")

        assert logger.name == "langconnect.server"
...
def test_sanitize_exception_redacts_credentials_from_messages() -> None:
    from langconnect.observability import sanitize_exception

    exc = RuntimeError(
        "failed postgresql://user:secret@localhost:5432/rag with token=abc123 and password=top"
    )
...
def test_dependency_error_fields_hide_python_exception_details() -> None:
    from langconnect.observability import dependency_error_fields

    exc = RuntimeError(
        "asyncpg.exceptions.CannotConnectNowError: failed postgresql://user:secret@db/rag"
    )
...
async def test_retry_async_retries_transient_failures_without_sleeping() -> None:
    from langconnect.observability import RetryPolicy, retry_async
...
def test_retry_sync_retries_transient_failures_without_sleeping() -> None:
    from langconnect.observability import RetryPolicy, retry_sync
...
def test_dependency_registry_tracks_required_and_degraded_states() -> None:
    from langconnect.observability import DependencyPolicy, DependencyRegistry
