...
def test_api_versioning_defaults_to_v1_when_config_is_missing(tmp_path: Path) -> None:
    ...
    config = load_api_versioning_config("user-service", config_path=tmp_path / "missing.toml")
...
""",
    ...
    config = load_api_versioning_config("user-service", config_path=config_path)
...
""",
    ...
    with pytest.raises(ValueError, match="default_version"):
        load_api_versioning_config("user-service", config_path=config_path)
