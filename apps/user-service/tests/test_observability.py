...
def test_json_log_formatter_emits_service_event_and_extra_fields() -> None:
    ...
    record = logging.LogRecord(
        name="src.core.database.startup",
        level=logging.INFO,
        pathname=__file__,
        lineno=12,
        msg="User service database migrations completed.",
        args=(),
        exc_info=None,
    )
    ...
    payload = json.loads(JsonLogFormatter(service_name="user-service").format(record))
    ...
    assert payload["message"] == "User service database migrations completed."
...
def test_text_log_formatter_uses_color_and_operator_fields() -> None:
    ...
    record = logging.LogRecord(
        ...
        pathname=__file__,
        lineno=12,
        msg="keycloak is unavailable during default admin bootstrap.",
        args=(),
        exc_info=None,
    )
    record.event = "startup.dependency.failed"
    record.dependency = "keycloak"
    record.operation = "ensure_default_admin"
    record.attempt = 5
    record.max_attempts = 5
    record.status = "failed"
    record.error_code = "dependency.connection_failed"

    line = TextLogFormatter(service_name="user-service").format(record)

    assert "\x1b[31mERROR\x1b[0m" in line
    assert "\x1b[31mfailed\x1b[0m" in line
    assert "user-service" in line
    assert "event=startup.dependency.failed" in line
    assert "dependency=keycloak" in line
    assert "operation=ensure_default_admin" in line
...
def test_configure_logging_installs_single_service_formatter() -> None:
    ...
    try:
        configure_logging(service_name="user-service", log_level="INFO", log_format="json")
        configure_logging(service_name="user-service", log_level="INFO", log_format="json")
...
def test_configure_logging_quiets_http_client_access_logs() -> None:
    ...
    try:
        configure_logging(service_name="user-service", log_level="INFO")
...
def test_sanitize_exception_redacts_credentials_from_messages() -> None:
    ...
    exc = RuntimeError(
        "failed postgresql://user:secret@localhost:5432/users with token=abc123 and password=top"
    )
...
def test_dependency_error_fields_hide_python_exception_details() -> None:
    ...
    exc = RuntimeError(
        "asyncpg.exceptions.CannotConnectNowError: failed postgresql://user:secret@db/users"
    )
...
def test_create_app_configures_service_logging(monkeypatch) -> None:
    ...
    monkeypatch.setattr(main, "configure_logging", fake_configure_logging, raising=False)
    ...
    assert calls == [
        {
            ...
            "log_level": main.get_settings().LOG_LEVEL,
        }
    ]
...
def test_dependency_registry_tracks_required_and_degraded_states() -> None:
    ...
    registry.record_degraded("keycloak_idp", hint="External IdP sync skipped.")

    assert registry.ready() is True
    snapshot = registry.snapshot()
    assert snapshot["postgres"].status == "ok"
    assert snapshot["postgres"].ready is True
    assert snapshot["keycloak_idp"].status == "degraded"
    assert snapshot["keycloak_idp"].ready is True
