...
async def create_key(
    name: str, user_id: Annotated[str, Depends(require_permission("api_key:create"))]
):
    return await get_api_key_controller().create_key(uuid.UUID(user_id), name)


@router.get("/")
async def list_keys(user_id: Annotated[str, Depends(require_permission("api_key:read"))]):
...
async def delete_key(
    key_id: str,
    user_id: Annotated[str, Depends(require_permission("api_key:delete"))],
):
