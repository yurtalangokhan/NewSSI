/**
 * Ported from Langflow (MIT) — src/frontend/src/pages/FlowPage/components/PageComponent/helpers/helper-lines.ts
 * Upstream: https://github.com/langflow-ai/langflow @ 3ec070e9
 * Adapted for Onyx: `Node` -> `CanvasNode`, otherwise verbatim — pure
 * geometry with no Langflow-specific coupling.
 *
...
export function getSnapPosition(
  ...
  const snapPosition = { ...draggingNode.position };

  if (helperLines.horizontal) {
    const height = draggingNode.measured?.height || nodeHeight;
    if (helperLines.horizontal.id.includes("top")) {
      ...
      snapPosition.y = helperLines.horizontal.position - height;
    } else if (helperLines.horizontal.id.includes("center")) {
      snapPosition.y = helperLines.horizontal.position - height / 2;
    }
  }

  if (helperLines.vertical) {
    const width = draggingNode.measured?.width || nodeWidth;
    if (helperLines.vertical.id.includes("left")) {
      ...
      snapPosition.x = helperLines.vertical.position - width;
    } else if (helperLines.vertical.id.includes("center")) {
      snapPosition.x = helperLines.vertical.position - width / 2;
    }
  }
