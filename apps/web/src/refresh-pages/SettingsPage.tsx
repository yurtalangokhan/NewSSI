...
import {
  ...
  ChatPreferencesSkeleton,
} from "@/refresh-components/skeletons/SettingsSkeletons";
...
import { EmailSettings } from "@/sections/settings/EmailSettings";
...
function GeneralSettings({
  ...
  const isNameDisabled = Boolean(user?.is_external_keycloak_user);
          ...
          <Card>
            ...
            >
              <InputTypeIn
                ...
                variant={isNameDisabled ? "disabled" : undefined}
                readOnly={isNameDisabled}
                showClearButton={!isNameDisabled}
                className={cn(
                  "w-60 shrink-0",
                  isNameDisabled
  ...
  const handleSaveShortcut = useCallback(
    async (index: number) => {
      ...
      try {
        if (shortcut.isNew) {
          // Create new shortcut
          const response = await authenticatedFetch("/api/input_prompt", {
            method: "POST",
          ...
          <InputLayouts.Horizontal
            ...
            description={t(
              personalizationValues.long_term_memory_enabled
                ...
                : "settings.chatPreferences.updateMemoriesRequiresReference"
            )}
          >
            <Switch
              disabled={!personalizationValues.long_term_memory_enabled}
              checked={
                personalizationValues.long_term_memory_enabled &&
...
function AccountsAccessSettings() {
  ...
  const showTokensSection =
    authType !== null &&
    (hasPermission("api_key:read") || hasPermission("api_key:create"));

  // Fetch PATs with SWR (safely without full-window redirect on auth errors)
  const {
    data: pats = [],
    mutate,
    error,
    ...
    showTokensSection && canCreateTokens ? "/api/user/pats" : null,
    async (url: string) => {
      const res = await authenticatedFetch(url, { redirectOnAuthError: false });
      if (!res.ok) {
  ...
  const createPAT = useCallback(async () => {
    ...
    setIsCreating(true);
    try {
      const response = await authenticatedFetch("/api/user/pats", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const deletePAT = useCallback(
    async (patId: number) => {
      try {
        const response = await authenticatedFetch(`/api/user/pats/${patId}`, {
          method: "DELETE",
          redirectOnAuthError: false,
        });
  ...
  const handleChangePassword = useCallback(
    async (values: {
      ...
      try {
        const response = await authenticatedFetch(
          "/api/user-service/users/me/password",
          {
            method: "POST",
            headers: {
