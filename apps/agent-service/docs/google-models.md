...
#### 3. Add the JSON key file to the [file-based credentials](file-based-credentials.md) path
Place the downloaded JSON file in the `privatecredentials/` of your project (e.g., `privatecredentials/service-account-key.json`).

Contents of the [file-based credentials](file-based-credentials.md) path are made available to your container at runtime in `/privatecredentials/`, but are excluded from git commits and Docker builds.
...
### Production note
This setup is ideal for development. In production, consider more secure alternatives. Some options are listed on the [file-based credentials](file-based-credentials.md) page.
