...
def test_json_log_formatter_emits_service_event_and_extra_fields() -> None:
    ...
    record = logging.LogRecord(
        name="core.db.startup",
        level=logging.INFO,
        pathname=__file__,
        lineno=12,
        msg="Agent service database migrations completed.",
        args=(),
        exc_info=None,
    )
    record.service = "agent-service"
    ...
    payload = json.loads(JsonLogFormatter(service_name="agent-service").format(record))
    ...
    assert payload["message"] == "Agent service database migrations completed."


def test_text_log_formatter_uses_color_and_operator_fields() -> None:
    from core.observability import TextLogFormatter
    ...
    record = logging.LogRecord(
        ...
        pathname=__file__,
        lineno=12,
        msg="tools-service is unavailable during startup connect.",
        args=(),
        exc_info=None,
    )
    record.event = "startup.dependency.failed"
    record.dependency = "tools-service"
    ...
    line = TextLogFormatter(service_name="agent-service").format(record)

    assert "\x1b[31mERROR\x1b[0m" in line
    assert "\x1b[31mfailed\x1b[0m" in line
    assert "agent-service" in line
    assert "event=startup.dependency.failed" in line
    assert "dependency=tools-service" in line
...
def test_configure_logging_installs_single_service_formatter() -> None:
    ...
    try:
        configure_logging(service_name="agent-service", log_level="INFO", log_format="json")
        configure_logging(service_name="agent-service", log_level="INFO", log_format="json")
...
def test_settings_expose_operator_friendly_log_format() -> None:
...
def test_sanitize_exception_redacts_credentials_from_messages() -> None:
    from core.observability import sanitize_exception

    exc = RuntimeError(
        "failed postgresql://user:secret@localhost:5432/agent with token=abc123 and password=top"
    )
...
def test_dependency_error_fields_hide_python_exception_details() -> None:
    from core.observability import dependency_error_fields

    exc = RuntimeError(
        "asyncpg.exceptions.CannotConnectNowError: failed postgresql://user:secret@db/agent"
    )
...
async def test_retry_async_retries_transient_failures_without_sleeping() -> None:
    from core.observability import RetryPolicy, retry_async
...
async def test_retry_async_raises_clear_dependency_error_after_exhaustion() -> None:
    from core.observability import DependencyStartupError, RetryPolicy, retry_async
...
def test_retry_sync_retries_transient_failures_without_sleeping() -> None:
    from core.observability import RetryPolicy, retry_sync
...
def test_dependency_registry_tracks_required_and_degraded_states() -> None:
    from core.observability import DependencyPolicy, DependencyRegistry

    registry = DependencyRegistry()
    registry.record_ok("postgres", policy=DependencyPolicy.REQUIRED)
    registry.record_degraded("neo4j", hint="Graph features disabled.")

    assert registry.ready() is True
    snapshot = registry.snapshot()
    assert snapshot["postgres"].status == "ok"
    assert snapshot["postgres"].ready is True
    assert snapshot["neo4j"].status == "degraded"
    assert snapshot["neo4j"].ready is True


def test_dependency_registry_not_ready_when_required_dependency_fails() -> None:
    from core.observability import DependencyPolicy, DependencyRegistry, DependencyStatus
