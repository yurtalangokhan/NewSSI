...
def test_api_versioning_defaults_to_v1_when_config_is_missing(tmp_path: Path) -> None:
    from core.api_versioning import load_api_versioning_config

    config = load_api_versioning_config("agent-service", config_path=tmp_path / "missing.toml")
...
""",
    ...
    config = load_api_versioning_config("agent-service", config_path=config_path)
...
""",
    ...
    with pytest.raises(ValueError, match="default_version"):
        load_api_versioning_config("agent-service", config_path=config_path)


def test_user_service_api_prefix_can_differ_from_local_service(tmp_path: Path) -> None:
    from core.api_versioning import load_api_prefix
