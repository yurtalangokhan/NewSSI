...
def get_kb_retriever(kb_id: str):
    """Create and return a Knowledge Base retriever instance."""
    if not kb_id:
        raise ValueError("AWS Knowledge Base ID is required")
...
async def acall_model(state: AgentState, config: RunnableConfig, *, store: BaseStore) -> AgentState:
    ...
    if long_term_memory and store and user_id:
        ...
        if memory_context:
            # Inject memory into the state for wrap_model to pick up
            if "kb_documents" in state:
                state = {**state, "kb_documents": state["kb_documents"] + memory_context}
            else:
                state = {**state, "kb_documents": memory_context}
