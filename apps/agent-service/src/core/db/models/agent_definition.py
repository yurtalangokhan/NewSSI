...
class AgentDefinitionModel(Base):
    ...
    # Canvas flow definition (nodes + edges + values). Additive, nullable —
    # existing rows stay NULL and keep running their classic graph_schema.
    # In P1 this IS the flow; P3 re-points it to a denormalized cache of the
    # published agent_flow_versions row (see .tmp/flow-canvas-design.md 5.1).
    flow_spec: Mapped[dict | None] = mapped_column(JSON, nullable=True, default=None)

    # What production traffic runs (P3, spec 5.1). NULL = never published;
    # the agent list shows a "Draft" badge for such agents.
    published_flow_version_id: Mapped[UUID | None] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("agent_flow_versions.id", ondelete="SET NULL", use_alter=True),
        nullable=True,
        default=None,
    )
    ...
    __table_args__ = (
        ...
        Index(
            "ix_agent_definitions_published_flow_version_id",
...
class FlowVersionStatus(str, Enum):
    """Lifecycle of one agent_flow_versions row (design spec 5.2)."""
...
class AgentFlowVersionModel(Base):
    """One version of a flow-backed agent's canvas definition.

    Append-only history: publishing never rewrites a row, it archives the
    previous published row and creates a new one; rollback does the same
    (design spec 5.2, 5.3). ``agent_definitions.flow_spec`` becomes a
    denormalized read cache of whichever row here is currently published —
    ...
    flow_spec: Mapped[dict] = mapped_column(JSON, nullable=False)
    status: Mapped[str] = mapped_column(String(20), nullable=False, default=FlowVersionStatus.DRAFT)
    ...
    __table_args__ = (
        Index("ix_agent_flow_versions_definition_id", "definition_id"),
        UniqueConstraint("definition_id", "version_no", name="uq_agent_flow_versions_def_version"),
        Index(
            "ix_agent_flow_versions_one_draft_per_definition",
            "definition_id",
            unique=True,
